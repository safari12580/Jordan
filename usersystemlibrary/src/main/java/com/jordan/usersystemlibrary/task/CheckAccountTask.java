...
public final class CheckAccountTask extends BaseTask {

    private String mAccount, mAccountType;
    private String mCheckCodeUrl;

    public CheckAccountTask(Context ctx, String remote_address, Handler main_handler,
                         String user_account, String account_type, String user_token,
                         boolean is_granted) {
        super(ctx, remote_address, user_token, main_handler, is_granted);
        mAccount = user_account;
        mAccountType = account_type;
        mCheckCodeUrl = mRemoteServerAddress + UserSystemConfig.URI_CHECK_ACCOUNT;
    }

    @Override
    public String doTask() {
        String main_json_str = UserSystemUtils.createCheckAccountMainRequest(mAccount, mAccountType);
    ...
    public void onSuccess(String result_json) {
        if (null != mMainHandler) {
            Message success_message = mMainHandler.obtainMessage(InnerMessageConfig.USER_CHECK_ACCOUNT_MESSAGE_SUCCESS);
    ...
    public void onFalse(String false_json) {
        if (null != mMainHandler) {
            Message success_message = mMainHandler.obtainMessage(InnerMessageConfig.USER_CHECK_ACCOUNT_MESSAGE_FALSE);
    ...
    public void onException() {
        if (null != mMainHandler) {
            Message success_message = mMainHandler.obtainMessage(InnerMessageConfig.USER_CHECK_ACCOUNT_MESSAGE_EXCEPTION);
            success_message.sendToTarget();
        }
    }
}
